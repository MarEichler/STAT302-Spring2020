...
admission_bar <- admission %>%
  ...
  pivot_longer(cols = c(-year), names_to = "status", values_to = "count" ) %>%
  mutate(
...
plot_line <-  ggplot(admission_line, aes(year, rate, color = status, shape = status)) +
  geom_point(size = 2) +
  geom_line(size = 1) +
  scale_color_manual(
      "Rate"
...
```{r L10-EX2-print-plots, fig.width = 9, fig.height=8, warning = FALSE}
plot_grid(
...
```{r L10-challenge-plot,  fig.width = 9, fig.height=4}
ggplot(admission_bar, aes(year)) +
  ...
  theme(
    ...
    , axis.text = element_text(color = NW_black50)
    , legend.text = element_text(color = NW_black80)
    , plot.title = element_text(color = NW_purple, family ="Georgia", face = "bold", hjust = 0.5)
    , plot.subtitle = element_text(color = NW_black80, family ="Georgia", hjust = 0.5)
